...
import "swiper/css";
import { Stit,product } from "../data/data"
              ...
              return (
                <SwiperSlide key={item.id}>
                  <Link to="" onMouseEnter={() => {
                    ...
                    <div className={`p_img`}>
                      <img src={`${process.env.PUBLIC_URL}/img/con0${item.id}_${(pnum === idx && hover) ? "on":"off"}.jpg`} alt="" />
                    </div>
                    <div className="txt">
                      <div className="tit">{item.name}</div>
                      <div className="des">&#8361; {item.price.toLocaleString()} 원</div>
